...
static int ad5592r_s_spi_enable_ref(struct ad5592r_s_state *st, u16 writeVal) {
    u16 tx = 0;
    u16 msg = 0;
    struct spi_transfer xfer[] = {
        {
...
static int ad5592r_s_read_channel(struct ad5592r_s_state *st, const struct iio_chan_spec *chan, int *val) {
    u16 rx = 0, temp = 0;
    int ret;
    ret = ad5592r_s_spi_write(st, AD5592R_S_CONF_ADC_SEQ_REG, AD5592R_S_ADC_SEQ(chan->channel));
    if(ret) {
                  ...
                  int *val2, long mask)
{
    ...
    case IIO_CHAN_INFO_RAW:
        if (st->en) {
            ret = ad5592r_s_read_channel(st, chan, val);
            if(ret) {
...
int ad5592r_s_write_raw(struct iio_dev *indio_dev,
                   struct iio_chan_spec const *chan, int val,
                   int val2, long mask)
{
    struct ad5592r_s_state *st = iio_priv(indio_dev);
...
static irqreturn_t ad5592r_s_trig_handler(int irq, void *p)
{
    ...
    {
        ret = ad5592r_s_read_channel(st, &chan[bit], &val);
        if (ret) {
            dev_err(&st->spi->dev, "FAILED to read the channel in handler");
            return IRQ_HANDLED;
        }
        buf[i] = val;
        i++;
...
static int ad5592r_s_probe(struct spi_device *spi)
{
    ...
    st->chan7 = 0;
    st->spi = spi;
    ret = ad5592r_s_spi_enable_ref(st, AD5592R_S_REG_REF);
    if(ret) {
        dev_err(&st->spi->dev, "Error writing at ADC_REF enable value");
        return ret;
    }
    ret = ad5592r_s_spi_write(st, AD5592R_S_CONF_ADC_CONFIG_REG, AD5592R_S_ADC_EN);
    if(ret) {
...
MODULE_AUTHOR("Pavel Robert <pavel.robert0110@gmail.com>");
