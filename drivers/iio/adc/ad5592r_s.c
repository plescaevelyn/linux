...
static int ad5592r_s_spi_write(struct iio_ad5592r_s_state *st, u8 reg,
			       u16 writeval)
{
	u16 tx = 0;
	u16 msg = 0;
	...
	tx = FIELD_PREP(AD5592R_S_WR_ADDR_MSK, reg) |
...
static int ad5592r_s_spi_read(struct iio_ad5592r_s_state *st, u8 reg,
			      u16 *readval)
{
	u16 tx = 0;
	u16 rx = 0;
...
static irqreturn_t iio_ad5592r_s_trig_handler(int irq, void *p)
{
   ...
   {
		ret =ad5592r_s_spi_write(st, AD5592R_S_ADC_SEQ, BIT(bit));
		if (ret) {
			dev_err(&st->spi->dev,
...
static int ad5592r_s_debugfs(struct iio_dev *indio_dev, unsigned int reg,
			     unsigned int writeval, unsigned *readval)
{
	struct iio_ad5592r_s_state *st = iio_priv(indio_dev);
	if (readval)
