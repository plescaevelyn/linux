...
static int iio_adc_emu_spi_write(struct iio_adc_emu_state *st,
                                ...
                                u8 writeval)
                                {
                                 ...
                                 tx = FIELD_PREP(IIO_ADC_ADDR_WR_MSK, reg) | FIELD_PREP(IIO_ADC_ADDR_VAL_MSK, writeval);
...
static int iio_adc_emu_debugfs(struct iio_dev *indio_dev, unsigned int reg, unsigned int writeval, unsigned *readval){
   struct iio_adc_emu_state *st=iio_priv(indio_dev);
   if(readval)
