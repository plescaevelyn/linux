                                 ...
                                 u8 writeval)
{
    ...
    tx = FIELD_PREP(IIO_ADC_ADDR_WRITE_MSK, reg) | FIELD_PREP(IIO_ADC_DATA_MSK, writeval);
...
static irqreturn_t iio_adc_emu_trigger_handler(int irq, void *p)
{
    ...
    if (ret)
    {
        dev_err(&st->spi->dev, "failed during conversion reg write in handler");
        return IRQ_HANDLED;
    }
    ...
    for_each_set_bit(bit, indio_dev->active_scan_mask, indio_dev->num_channels)
    {
        ...
        if (ret)
        {
            dev_err(&st->spi->dev, "failed during reading channel %d high in handler", bit);
            return IRQ_HANDLED;
        }
        ret = iio_adc_emu_spi_read(st, IIO_ADC_EMU_REG_CHAN_LOW(bit), &low);
        if (ret)
        {
            dev_err(&st->spi->dev, "failed during reading channel %d low in handler", bit);
            return IRQ_HANDLED;
        }
...
static int iio_adc_emu_probe(struct spi_device* spi)
{
    ...
    ret = devm_iio_triggered_buffer_setup_ext(&spi->dev, indio_dev, NULL, iio_adc_emu_trigger_handler, IIO_BUFFER_DIRECTION_IN, NULL, NULL);
    if (ret)
    {
        dev_err(&spi->dev, "Error during buffer setup");
        return ret;
    }
