...
static int iio_adc_emu_read_chan(struct iio_adc_emu_state *st, const struct iio_chan_spec *chan, int *val)
{ 
    int ret;
    u8 high;
    u8 low;
    ...
    if (ret)
    {
        dev_err(&st->spi->dev, "FAILED conversion read chan high");
        return ret;
    }
    ...
    if (ret)
    {
        dev_err(&st->spi->dev, "FAILED conversion read chan low");
        return ret;
    }
    *val = (high << 8) | low;
