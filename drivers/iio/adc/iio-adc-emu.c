                                ...
                                u8 *readval)
{
    ...
    tx = IIO_ADC_EMU_RD_MSK | FIELD_PREP(IIO_ADC_ADDR_RD_MSK, reg);

    int ret = spi_sync_transfer(st->spi, xfer, 2);
    if(ret){
                                 ...
                                 int *val)
{
        ...
        if(ret){
            dev_err(&st->spi->dev, "FAILED channel high");
            return ret;
        }     

        ret = iio_adc_emu_spi_read(st,IIO_ADC_EMU_REG_CHAN_LOW(chan->channel),&low);
        if(ret){
            dev_err(&st->spi->dev, "FAILED channel low");
            return ret;
        }     
...
static irqreturn_t iio_adc_emu_trig_handler(int irq, void *p){
    ...
    for_each_set_bit(bit,indio_dev->active_scan_mask,2){
...
MODULE_AUTHOR("Maris Radu-Ioan");
MODULE_DESCRIPTION("Analog Devices IIO ADC EMU");
